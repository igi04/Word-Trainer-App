...
class FlashcardsWindow(BaseWindow):
    ...
    def build_interface(self):
        ...
        self.flashcard_button = Button(frame, command=self.on_click_flashcard_button, text="", font=("helvetica", 32), wraplength=self.window.winfo_width() * 0.2 )
    ...
    def on_click_no(self, event=None):
        ...
        if result:
            ...
            self.flashcard_button.config(text="All done! 🎉")
            self.fail_button.config(state="disabled")
            self.success_button.config(state="disabled")
