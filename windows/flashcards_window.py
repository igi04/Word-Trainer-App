...
class FlashcardsWindow(BaseWindow):
    def __init__(self,app_data, learning_mode):
        ...
        #Bind buttons to ease flashcards work
    ...
    def build_interface(self):
        ...
        self.flashcard_button = Button(frame,
                                       command=self.on_click_flashcard_button,
                                       text="",
                                       font=("helvetica", 32),
                                       wraplength=300,
                                       justify="center")
        ...
        self.fail_button = Button(frame,
                                  command=self.on_click_no,
                                  text="No",
                                  font=("helvetica", 14))
        self.fail_button.pack(expand=True, fill="x", side=LEFT, padx=5)

        self.success_button = Button(frame,
                                     command=self.on_click_yes,
                                     text="Yes",
                                     font=("helvetica", 14))
    ...
    def on_click_flashcard_button(self, event=None):
        if self.flashcard_engine.translation_on:
            self.flashcard_button.config(text=self.flashcard_engine.get_word(),
                                         fg="#f8f9fa")

        else:
            self.flashcard_button.config(text=self.flashcard_engine.get_translation(),
                                         fg="#adb5bd")
    ...
    def on_click_yes(self, event=None):
        ...
        if result:
            self.flashcard_button.config(text=result,
                                         fg="#f8f9fa")
            self.progress_update()
        else:
            self.progress_update()

            #Disable work of widgets after finish flashcards
    ...
    def show_word_on_the_beginning(self):
        ...
        self.flashcard_button.config(text=word,
                                     fg="#f8f9fa",
    ...
    #Set wrap length for flashcards button text
